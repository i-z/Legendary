...
namespace Legendary
{
    public class WebUser
    {
        ...
        public static WebUser Get(HttpContext context, UserModel userModel)
        {
            WebUser user;
            if (context.Session.Get("user") != null)
            {
                user = JsonSerializer.Deserialize<WebUser>(GetString(context.Session.Get("user")));
            }
            else
            {
                user = new WebUser().SetContext(context);
            }
            user.Init(context, userModel);
            return user;
        }

        public WebUser SetContext(HttpContext context)
        {
            Context = context;
            return this;
        }

        public virtual void Init(HttpContext context, UserModel userModel)
        {
            //generate visitor id
            Context = context;
            ...
            {
                var user = userModel.AuthenticateUser(context.Request.Cookies["authId"]);
                if (user != null)
                {
        ...
        public void LogIn(UserModel userModel, int userId, string email, string name, DateTime datecreated, string displayName = "", short userType = 1, bool photo = false)
        {
            this.userId = userId;
